...
def print_header(fout):
    ...
    fout.write('\n')
    fout.write('vector<float> thresholds_pt_iso;\n')
    fout.write('vector<TString> ptbins_iso;\n')
    ...
    for b in range(1, th.GetNbinsY()+1):
        fout.write('\tthresholds_pt_iso.push_back('+str(th.GetYaxis().GetBinLowEdge(b))+');\n')
        fout.write('\tptbins_iso.push_back("pt'+str(b-1)+'");\n')
    tf.Close()
    fout.write('}\n')
    fout.write('\n')
    fout.write('vector<float> thresholds_eta_iso;\n')
    fout.write('vector<TString> etabins_iso;\n')
    fout.write('inline void set_etabins_iso() {\n')
    tf = ROOT.TFile(indir+"/Efficiencies_muon_generalTracks_Z_Run2018_UL_ISO.root")
    th   = tf.Get("NUM_LooseRelTkIso_DEN_HighPtIDandIPCut_abseta_pt").Clone("thist")
    for b in range(1, th.GetNbinsX()+1):
        fout.write('\tthresholds_eta_iso.push_back('+str(th.GetXaxis().GetBinLowEdge(b))+');\n')
        fout.write('\tetabins_iso.push_back("eta'+str(b-1)+'");\n')
    tf.Close()
    fout.write('}\n')
    fout.write('\n')
...
def print_cc(fout,fname):
    ...
    fout.write('\n')
    fout.write('TString get_muonIsoSFPtBin(const float pt) {\n')
    fout.write('\tfor ( unsigned int b=thresholds_pt_iso.size()-1; b>=1; b-- ) {\n')
    fout.write('\t\tif ( pt > thresholds_pt_iso.at(b) ) {\n')
    fout.write('\t\t\treturn ptbins_iso.at(b);\n')
    fout.write('\t\t}\n')
    fout.write('\t}\n')
    fout.write('\treturn ptbins_iso.at(0);\n')
    fout.write('}\n')
    fout.write('\n')
    fout.write('TString get_muonIsoSFEtaBin(const float abseta) {\n')
    fout.write('\tfor ( unsigned int b=thresholds_eta_iso.size()-1; b>=1; b-- ) {\n')
    fout.write('\t\tif ( abseta > thresholds_eta_iso.at(b) ) {\n')
    fout.write('\t\t\treturn etabins_iso.at(b);\n')
    fout.write('\t\t}\n')
    fout.write('\t}\n')
    fout.write('\treturn etabins_iso.at(0);\n')
    fout.write('}\n')
    fout.write('\n')
