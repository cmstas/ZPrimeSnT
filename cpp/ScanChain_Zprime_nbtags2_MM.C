...
int ScanChain(TChain *ch, double genEventSumw, TString year, TString process) {
  ...
  */
  ...
  TFile* fout = new TFile("temp_data/output_"+process+"_"+year+"nb2_MM.root", "RECREATE");
  float m_ll, wgt, weighted_evts;
  bool flag_MM, flag_MM_1b;

  TTree tree_out("tree","");
  tree_out.Branch("m_ll",&m_ll);
  tree_out.Branch("wgt",&wgt);
  tree_out.Branch("weighted_evts",&weighted_evts);
  tree_out.Branch("flag_MM",&flag_MM);
  tree_out.Branch("flag_MM_1b",&flag_MM_1b);
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) {
	...
	if ( runnb >= 319077 )
      ...
      std::vector<unsigned int> bjet_idx_loose_20;
      std::vector<unsigned int> bjet_idx_med_20;
      std::vector<unsigned int> bjet_idx_tight_20;;
      ...
      for ( unsigned int jet = 0; jet < nt.nJet(); jet++ ){
        ...
        if (nt.Jet_pt().at(jet)<20. || std::abs(nt.Jet_eta().at(jet))>=2.5 || nt.Jet_jetId().at(jet)==0) continue;
      ...
      //if (bjet_idx_loose_20.size()<2) continue;
      ...
      // Only consider events where we have 1 medium btag with pT > 20 GeV
      ...
      flag_MM = false;
      float min_mlb_mm = -1;
      float min_mlb_mm_1b = -1;
    
      if ( bjet_idx_med_20.size() == 1 ){
           //min_mlb_tl_1b = 1e9;
           auto bjet_p4 = nt.Jet_p4().at(bjet_idx_med_20[0]);
           float m_mu1_mb = (leadingMu_p4+bjet_p4).M();
           float m_mu2_mb = (subleadingMu_p4+bjet_p4).M();       
           min_mlb_mm_1b = 1e9;
           min_mlb_mm_1b = std::min(min_mlb_mm_1b,std::min(m_mu1_mb,m_mu2_mb));
           if (min_mlb_mm_1b > 175.0){
               flag_MM_1b = true;
           }
           
      }

      // Now consider the case where number of medium b jets is greater than one.......

      if ( bjet_idx_med_20.size() > 1 ){
           min_mlb_mm = 1e9;
           float min_mlb_m = 1e9;
           for (int j = 0; j < bjet_idx_med_20.size(); j++ ){
                auto bjet_p4_2 = nt.Jet_p4().at(bjet_idx_med_20[j]);
                float m_mu1_mb_1 = (leadingMu_p4+bjet_p4_2).M();
                float m_mu2_mb_1 = (subleadingMu_p4+bjet_p4_2).M();
                min_mlb_m = std::min(min_mlb_m,std::min(m_mu1_mb_1,m_mu2_mb_1));
           }
           min_mlb_mm = min_mlb_m;
           if (min_mlb_mm > 175.0 ){
               flag_MM = true;
           }
      ...
      pass_any_flag = ( flag_MM || flag_MM_1b );
