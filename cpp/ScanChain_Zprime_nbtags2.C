...
int ScanChain(TChain *ch, double genEventSumw, TString year, TString process) {
  ...
  */
  ...
  tree_out.Branch("flag_TT",&flag_TT);
  tree_out.Branch("flag_TT_1b",&flag_TT_1b);
  tree_out.Branch("flag_TM",&flag_TM);
  tree_out.Branch("flag_TM_1b",&flag_TM_1b);
  tree_out.Branch("flag_TL",&flag_TL);
  tree_out.Branch("flag_TL_1b",&flag_TL_1b);
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) {
	...
	if ( runnb >= 319077 )
      ...
      // Only consider events where we have 1 tight btag with pT > 20 GeV
      ...
      if ( bjet_idx_med_20.size() > 1 ){
           ...
           for (int j = 0; j < bjet_idx_med_20.size(); j++ ){
                ...
                float m_mu1_mb = (leadingMu_p4+bjet_p4_2).M();
